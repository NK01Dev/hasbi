...
class AddGoalView extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final currentController = useTextEditingController(text: ""); // Removed current amount edit for simplicity in MVP, or can add back if needed
    ...
    useEffect(() {
      if (state.id != null) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
            ...
            if (state.targetAmount.isNotEmpty && targetController.text != state.targetAmount) {
                targetController.text = state.targetAmount;
            }
            if (state.currentAmount.isNotEmpty && currentController.text != state.currentAmount) {
        ...
        actions: [
          TextButton(
            ...
            child: state.isLoading
                ...
                : Text("Save", style: TextStyleHelper.textStyle16(color: AppColors.primary, fontWeight: FontWeight.w600)),
          )
        ],
      ),
      body: _buildBody(
  ...
  Widget _buildBody(
      ...
      if (state.errorMessage != null && state.id == null && isEditing) {
          return custom.ErrorWidget(errorMessage: state.errorMessage!, onRetry: () => notifier.loadGoal(goalId!));
      }
  ...
  Widget _buildStatusDropdown(BuildContext context, GoalStatus currentStatus, AddGoalNotifier notifier) {
      return Container(
  ...
  Color _getStatusColor(GoalStatus status) {
      switch (status) {
          case GoalStatus.active: return AppColors.primary;
          case GoalStatus.reached: return AppColors.success;
          case GoalStatus.failed: return AppColors.error;
